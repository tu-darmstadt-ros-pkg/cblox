...
class cblox_factory {
  ...
  //TODO template based and non template based functions (create TsdfRgbSensor, etc.)

    template <typename SubmapType, typename CollisionVoxelType>
    static std::shared_ptr<RGBSensor<SubmapType, CollisionVoxelType>> createRGBSensor();

    template <typename SubmapType, typename GeometryVoxelType>
    static std::shared_ptr<LIDARSensor<SubmapType, GeometryVoxelType>> createLIDARSensor();

    //TODO needed, do different
    //static std::shared_ptr<GenericSubmapCollectionIntegrator> createGenericSubmapCollectionIntegrator();
